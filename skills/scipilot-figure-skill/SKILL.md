...
description: 分析科研数据并选择合适图型、分组和统计表达。用于绘图前判断或图型建议；实际绘制使用 nature-figure，保留用户后端选择和 R 默认。
---
...
科研绘制、导出和视觉检查遵循 `nature-figure`。沿用已明确的用户后端，未指定时使用 R。下文 Python 脚本可用于非绘图数据检查；Python 绘图、预览和样式配方仅在选定 Python 时使用。R 路径用对应 R 实现，不为预览切换后端。

## 核心工作流

先读取数据并明确变量含义，再选图和绘制。已有上下文与检查结果直接复用，步骤按任务需要安排。
...
用当前可用的数据处理环境检查列类型、样本量、缺失值、分组和分布。R 出图可直接用 R 完成这些检查；不为数据剖析安装另一套语言依赖。已有 Python 和 pandas 时，可选用 `scripts/profile_data.py`：
...
- 用户询问图型时给推荐和理由；已委托出图时直接选择适合数据的图型并制作，备选仅在有实质取舍时提供。
...
未指定期刊时沿用论文模板，或采用通用科研图尺寸并说明；只有正式投稿规范存在关键歧义时才补问。
...
1. **第 0 步**：从数据说明和上下文确定比较目标；变量含义或目标存在实质歧义时才补问。
2. **第 1 步**：用已选后端或现有数据工具检查数据；Python 和 pandas 已可用时可运行 `scripts/profile_data.py`。
3. **第 2 步**：读报告里"列类型 + 样本量 + 分组结构"，结合用户的论证目标查 `chart_selection.md` 决策。**主动给出推荐 + 理由 + 1-2 备选**
4. 用户已委托出图时直接继续：采用适合数据的图型，按 `nature-figure` 的后端规则绘制、自检和导出；不重复索取图型确认。

复用已明确的信息与有效检查，不省略数据含义和实际出图检查。
